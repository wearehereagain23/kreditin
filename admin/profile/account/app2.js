 ...
 * Reads the first row from the 'kreditin_admin' table
 */
async function loadAdminSettings() {
    ...
    const { data, error } = await supabase.from('kreditin_admin').select('*').limit(1).single();
...
settingsForm?.addEventListener('submit', async (e) => {
    ...
    const { data: existing } = await supabase.from('kreditin_admin').select('id').limit(1);
