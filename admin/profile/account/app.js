...
async function initProfile() {
    ...
    // Fetch initial data
    const { data, error } = await supabase.from('kreditin_users').select('*').eq('uuid', USERID).single();
    ...
    supabase.channel('profile-sync')
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'kreditin_users', filter: `uuid=eq.${USERID}` },
...
document.getElementById('profileForm')?.addEventListener('submit', (ev) => safe(async () => {
    ...
    const { error } = await supabase.from('kreditin_users').update({
...
document.getElementById('deleteUser')?.addEventListener('click', () => safe(async () => {
    ...
    if (res.isConfirmed) {
        showSpinnerModal();
        await supabase.from('kreditin_users').delete().eq('uuid', USERID);
        await supabase.from('kreditin_history').delete().eq('uuid', USERID);
        hideSpinnerModal();
